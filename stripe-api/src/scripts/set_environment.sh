...
aws lambda update-function-configuration --region $AWS_REGION --function-name $ACTION_FUNCTION_NAME --environment "Variables={DNS_DOMAIN=$DNS_DOMAIN,DEPLOY_STAGE=$DEPLOY_STAGE,STRIPE_SECRET_KEY=$STRIPE_SECRET_KEY,STRIPE_PLAN_BASIC_PRICE_ID=$STRIPE_PLAN_BASIC_PRICE_ID}"
