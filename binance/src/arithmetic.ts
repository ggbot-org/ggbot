...
const maxNumOfDecimals = (values: BinanceDecimal[]): number => values.reduce<number>((max, num) => Math.max(max, numOfDecimals(num)), 0)

export const add: BinanceBinaryOperator = (a, b, precision) => (Number(a) + Number(b)).toFixed(precision ?? maxNumOfDecimals([a, b]))

export const sub: BinanceBinaryOperator = (a, b, precision) => (Number(a) - Number(b)).toFixed(precision ?? maxNumOfDecimals([a, b]))
...
export const neg: BinanceUnaryOperator = (a) => (-Number(a)).toFixed(numOfDecimals(a))

export const lessThan: BinanceComparisonOperator = (a, b) => Number(a) < Number(b)

export const greaterThan: BinanceComparisonOperator = (a, b) => Number(a) > Number(b)
