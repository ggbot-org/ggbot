...
export class BinanceExchange {
	...
	async exchangeInfo(): Promise<BinanceExchangeInfo> {
		...
		const { serverTime, symbols } =
			await this.connector.request<BinanceExchangeInfo>(
				'GET',
				'/api/v3/exchangeInfo'
			)
		const data = {
	...
	async klines(
		...
		const klines = await this.connector.request<BinanceKline[]>(
			'GET',
			'/api/v3/klines',
			{ symbol, interval, ...optionalParameters }
		)
		// Cache all klines found.
		if (cache)
			for (const kline of klines) await cache.setKline(symbol, interval, kline)
		return klines
	}
