...
export async function extractCommonParametersFromFlow(
			...
			nodesCatalog: {
				...
				[BooleanParameter.kind]: class MockedBooleanParameter extends DflowNode {
					...
					run() {
						...
						// Set parameter
						parameters.push({ kind: BooleanParameter.kind, key, defaultValue })
					}
				},
				...
				[StringParameter.kind]: class MockedStringParameter extends DflowNode {
					...
					run() {
						...
						this.output(0).data = value
						// Set parameter
						parameters.push({ kind: StringParameter.kind, key, defaultValue })
					}
				},
			},
		},
