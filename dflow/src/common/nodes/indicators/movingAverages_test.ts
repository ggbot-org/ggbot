	...
	assertDeepEqual<ExponentialMovingAverageInput, ReturnType<ExponentialMovingAverage>>(
		...
		[
			{ input: { values: [], period: 1 }, output: [] },
			{
				...
				output: [
					81.59, 81.41333333, 81.89888889, 82.26592593, 82.71395062, 82.85930041, 82.85286694, 83.23191129, 83.67127419, 83.90084946, 84.44389964, 85.14259976, 85.72506651, 86.40671101, 86.70114067
				]
			}
		]
	)
	...
	assertDeepEqual<SimpleMovingAverageInput, ReturnType<SimpleMovingAverage>>(
		...
		[
			{ input: { values: [], period: 1 }, output: [] },
			{
				...
				output: [
					82.426, 82.738, 83.094, 83.318, 83.628, 83.778, 84.254, 84.994, 85.574, 86.218, 86.804
				]
			}
		]
	)
...
test("Wilder's smoothing", () => {
	assertDeepEqual<WilderSmoothingInput, ReturnType<WilderSmoothing>>(
		({ values, period }: WilderSmoothingInput) => wilderSmoothing(values, period),
		[
			{
			...
			},
			{
				...
				output: [
					82.426, 82.5708, 82.62464, 82.897712, 83.2281696, 83.45453568, 83.86962854, 84.40370283, 84.90096226, 85.47476981, 85.83781585
				]
			}
		]
	)
