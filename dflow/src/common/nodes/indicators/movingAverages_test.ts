...
test('Exponential Moving Average', () => {
	...
	for (const { input, output } of testData) {
		assert.deepEqual(exponentialMovingAverage(input.values, input.period), output)
	}
})
...
test('Simple Moving Average', () => {
	...
	const testData: TestData = [
		{ input: { values: [], period: 1 }, output: [] },
		{
			...
			output: [82.426, 82.738, 83.094, 83.318, 83.628, 83.778, 84.254, 84.994, 85.574, 86.218, 86.804],
		},
	]
