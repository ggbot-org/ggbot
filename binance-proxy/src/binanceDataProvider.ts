import { BinanceProxyApiDataProvider, CreateBinanceOrderInput } from "@workspace/api"
import {BinanceAccountInformation, BinanceApiKeyPermission, BinanceOrderRespFULL} from "@workspace/binance"
...
export class BinanceDataProvider implements BinanceProxyApiDataProvider {
	...
	createBinanceOrder({symbol, side, type, orderOptions}: CreateBinanceOrderInput): Promise<BinanceOrderRespFULL> {
		return this.binance.newOrder(symbol, side, type, orderOptions)
	}

	createBinanceOrderTest({symbol, side, type, orderOptions}: CreateBinanceOrderInput): Promise<BinanceOrderRespFULL> {
