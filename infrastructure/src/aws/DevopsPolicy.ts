	...
	implements IamPolicyDocument<StatementName, StatementAction>
{
	...
	> {
		return {
			deployWebapp: ['s3:DeleteObject', 's3:ListBucket', 's3:PutObject'],
			downloadDataBuckets: ['s3:ListBucket', 's3:GetObject'],
			manageApiLambdas: [
				'lambda:CreateFunction',
				'lambda:UpdateFunctionCode',
				'lambda:UpdateFunctionConfiguration',
	...
	> {
		return {
			...
			manageApiLambdas: LambdaFunction.arn(
				ENV.AWS_DATA_REGION(),
				`${ApiLambda.apiNamePrefix()}*`
			),
			manageLogGroups: '*',
