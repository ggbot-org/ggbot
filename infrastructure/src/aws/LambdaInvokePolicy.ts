...
const statementNames = ['invokeLambda'] as const
type StatementName = (typeof statementNames)[number]
type StatementAction = Extract<IamAction, 'lambda:InvokeFunction'>
...
export class LambdaInvokePolicy extends IamPolicy {
	constructor() {
		super(
			ENV.AWS_ACCOUNT_ID(),
			ENV.AWS_DATA_REGION(),
			`${ENV.PROJECT_SHORT_NAME()}-lambda-invoke-policy`
		)
	}
	...
	> {
		return {
			invokeLambda: ['lambda:InvokeFunction'],
		}
	}
