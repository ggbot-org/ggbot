...
export class WebappBaseURL extends URL {
	constructor(fqdn: FQDN) {
		super(fqdn.deployStage === 'local' ? 'http://localhost:8000' : `https://${fqdn.webappDomain}`)
	}
}
...
function appendAccountKeyToURLSearchParams({ accountId }: AccountKey, url: URL) {
	url.searchParams.append('accountId', accountId)
	return url
}

function appendStrategyKeyToURLSearchParams({ strategyId, strategyKind }: StrategyKey, url: URL) {
 ...
 *
 * ```ts
 * import { FQDN, WebappURLs, WebappBaseURL	} from "@workspace/locators"
 *
...
export class WebappURLs {
	...
	strategy(strategyKey?: StrategyKey) {
		...
		return strategyKey ? appendStrategyKeyToURLSearchParams(strategyKey, url) : url
	}
}
