...
aws lambda update-function-configuration --region $AWS_REGION --function-name $ORDER_FUNCTION_NAME --environment "Variables={AWS_ACCOUNT_ID=${AWS_ACCOUNT_ID},AWS_DATA_REGION=${AWS_DATA_REGION},DEPLOY_STAGE=$DEPLOY_STAGE,DNS_DOMAIN=${DNS_DOMAIN},UTRUST_API_KEY=$UTRUST_API_KEY}"
