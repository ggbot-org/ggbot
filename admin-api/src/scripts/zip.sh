...
for LAMBDA in action; do
	...
	# Copy internal deps.
	for WORKSPACE in api api-gateway arithmetic authentication aws-s3 binance binance-client cache database dflow env email-messages http locators logging models s3-data-bucket; do
