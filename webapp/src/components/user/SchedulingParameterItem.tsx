...
export function SchedulingParameterItem({
	...
	const onChange = useCallback<
		ChangeEventHandler<
			HTMLInputElement & { value: SchedulingParameterItemProps['paramValue'] }
		>
	>(
		(event) => {
	...
	useEffect(() => {
		...
		if (kind === PercentageParameter.kind) {
			if (
				...
				parsePercentage(paramValue) !== undefined
			)
				return
		}
		if (kind === SymbolParameter.kind) {
	...
	const placeholder = useMemo(() => {
		if (PercentageParameter.kind === kind) {
			if (typeof defaultParamValue === 'number')
				return `${defaultParamValue * 100}%`
		}
