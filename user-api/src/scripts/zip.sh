...
echo '{ "name": "lambda", "type": "module", "dependencies": { "@aws-sdk/client-ec2": "3.379.1", "@aws-sdk/client-iam": "3.379.1", "@aws-sdk/client-s3": "3.379.1", "@aws-sdk/client-ses": "3.379.1", "jsonwebtoken": "9.0.1", "dflow": "0.42.0", "flow-view": "6.0.1", "minimal-time-helpers": "0.1.0" } }' > temp/package.json
cd temp
npm install
cd -
...
for INTERNAL_DEP in api-gateway arithmetic authentication aws binance binance-client cache database dflow env email-messages http infrastructure locators models test-data type-utils; do
...
for WORKSPACE in api logging; do
