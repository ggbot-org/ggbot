...
aws lambda update-function-configuration --region $AWS_REGION --function-name $ACTION_FUNCTION_NAME --environment "Variables={AWS_ACCOUNT_ID=$AWS_ACCOUNT_ID,AWS_DATA_REGION=$AWS_DATA_REGION,BINANCE_PROXY_BASE_URL=$BINANCE_PROXY_BASE_URL,DEPLOY_STAGE=$DEPLOY_STAGE,DNS_DOMAIN=$DNS_DOMAIN,AUTHENTICATION_SECRET=$AUTHENTICATION_SECRET}"
